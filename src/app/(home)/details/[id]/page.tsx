...
import ExploreMore from "@/components/details/ExploreMore";
import WhatsAppChat from "@/components/common/WhatsApp";
import { WHATSAPP_NUMBER } from "@/config";

import ProductImageGallery from "./_components/ProductImageGallery";
import ProductTitleSkeleton from "./_components/ProductTitleSkeleton";
import VariantSelectorSkeleton from "./_components/VariantSelectorSkeleton";
import ProductDescription from "./_components/ProductDescription";

import { useAppDispatch } from "@/redux/store";
import { addItem } from "@/redux/features/cart-slice"; // ⬅️ same logic as inspiration
import { useGetProductBySlugQuery } from "@/redux/api/publicApi";
...
export default function ProductDetailsPage() {
  ...
  const doAddToCart = async (): Promise<boolean> => {
    ...
    try {
      ...
      toast.success(
        `Added: ${productDetails?.name ?? "Product"}${suffix ? ` • ${suffix}` : ""
        } × ${quantity}`
      );
      return true;
    } catch {
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16 mb-16">
              ...
              <div className="space-y-6">
                        ...
                        {options.map((option) => {
                          ...
                          return (
                            <button
                              ...
                              className={`px-4 text-xs py-2 border-2 rounded-lg font-medium transition-all duration-200 flex items-center justify-center min-w-[64px] ${selected
                ...
                <div className="md:flex md:gap-4">
                  ...
                  >
                    {isAdding ? "Processing..." : hasPreOrder ? "Pre-Order" : "Buy Now"}
                  </button>
                </div>
                  ...
                  <div className="flex items-center gap-3 p-3 bg-blue-50 rounded-lg">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-blue-700">
                        {isFreeDelivery
                          ? deliveryTime
                          : `Tk ${shippingCost}${shippingCostPerUnit > 0
