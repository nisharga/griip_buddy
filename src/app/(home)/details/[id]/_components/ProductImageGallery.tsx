      ...
      <div className="flex gap-4">
        {images.map((image, index) => (
          <button
            ...
            className={`relative w-[100px] aspect-square bg-white rounded-xl overflow-hidden border-2 transition-all ${
              selectedImage === index
    ...
    <div className="space-y-4 animate-pulse space-x-4 !w-[95%]">
      ...
      <div className="flex gap-4">
        {[...Array(3)].map((_, index) => (
          <div
            key={index}
            className="w-[100px] aspect-square bg-gray-200 rounded-lg"
          ></div>
        ))}
      </div>
    </div>
