...
import { Minus, Plus, Trash2, Truck, ShoppingBag, CreditCard } from "lucide-react";
            ...
            <CardContent>
                ...
                <div className="space-y-3">
                    {isLoading ? (
                        ...
                        <div className="rounded-lg bg-gray-50 p-4 text-sm text-gray-700">Your cart is empty.</div>
                    ) : (
                        items.map((it) => (
                            <motion.div
                                key={it.id}
                            ...
                            >
                                ...
                                <div className="min-w-0 flex-1">
                                    <p className="line-clamp-2 text-sm font-medium text-gray-900">{it.product.name}</p>
                ...
                <div className="mt-4 space-y-2">
                    ...
                    {perLineShippingTotal > 0 && (
                        <div className="flex justify-between text-sm">
                            ...
                            <span className="font-medium">Tk {perLineShippingTotal.toLocaleString()}</span>
                        </div>
                    )}
                    ...
                    <div className="flex justify-between text-sm">
                        ...
                        <span className="font-medium">
                            Tk {deliveryCharge.toLocaleString()} {isFetching ? "(updating…)" : ""}
                        </span>
                    </div>

                    <div className="flex justify-between text-sm">
                        <span className="text-gray-600">Discounts</span>
                        <span className="font-medium">-Tk {totalDiscount.toLocaleString()}</span>
                    </div>
                    ...
                    <div className="flex justify-between text-base font-semibold">
                        <span>Total</span>
                        <span className="text-[#ff5c00]">Tk {totalAmount.toLocaleString()}</span>
                    </div>

                    <div className="flex justify-between text-sm text-gray-600">
                        <span>Pay now</span>
                        <span className="font-medium text-gray-900">Tk {payableNow.toLocaleString()}</span>
                    </div>
                </div>
                ...
                {!!onPlaceOrder && (
                    <Button
                        ...
                        className="mt-5 w-full rounded-lg bg-[#ff5c00] py-6 text-base font-semibold text-white shadow-lg transition hover:translate-y-[-1px] hover:bg-[#ff5c00]/90"
                    >
                        {ctaLabel}
                    </Button>
                )}
                ...
                <div className="mt-3 flex items-center justify-between text-sm">
                    <Link href="/cart" className="text-[#ff5c00] hover:underline flex items-center gap-2">
                        <ShoppingBag className="h-4 w-4" /> Review Cart
                    </Link>
                    <Link href="/" className="text-[#ff5c00] hover:underline flex items-center gap-2">
