...
import { CheckCircle2, ChevronDown, PlusCircle, Info } from "lucide-react";
...
import { removeItem as removeLocalItem, updateQty as updateLocalQty, selectCartItems } from "@/redux/features/cart-slice";
...
import ManualPayment, { ManualPaymentFields } from "./_components/ManualPayment";
...
type ApiVariant = {
  ...
  sku?: string;
  barcode?: string;
  product?: string | { _id: string; name?: string; slug?: string; thumbnail?: string };
};
...
const CheckoutPageMain: React.FC = () => {
  ...
  const { data: productsRes, isLoading: cartLoading, isFetching, isError } = useGetProductByIdsQuery(
  ...
  const items: OrderItem[] = useMemo(() => {
    ...
    for (const line of lines) {
      ...
      const price = (variant?.sale_price ?? variant?.regular_price ?? line.priceSnapshot ?? 0) as number;
      ...
      const lineShipping = qty > 0 ? shippingBase + Math.max(0, qty - 1) * shippingPerUnit : 0;
  ...
  const sub_total = useMemo(() => items.reduce((s, it) => s + it.unitPrice * it.quantity, 0), [items]);

  
  const [form, setForm] = useState<DeliveryForm>(initialForm);
  const [bdData, setBdData] = useState<BDDataShape>(bangladeshDivisions as unknown as BDDataShape);
  const divisions = useMemo(() => Object.keys(bdData), [bdData]);
  const districts = useMemo(() => (form.division ? bdData[form.division]?.map((d) => d.জেলা) ?? [] : []), [form.division, bdData]);
  const thanas = useMemo(() => {
    if (!form.division || !form.district) return [];
    const districtObj = (bdData[form.division] || []).find((d) => d.জেলা === form.district);
  ...
  const perLineShippingTotal = useMemo(() => items.reduce((s, it) => s + it.lineShipping, 0), [items]);
  ...
  const [pay, setPay] = useState<ManualPaymentFields>({ senderPhone: "", trxId: "", amount: 0 });
  const COD_ADVANCE = 200;
  const expectedPayNow = form.payment_type === "cod" ? COD_ADVANCE : total_amount;
  ...
  const createDivision = (name: string) => setBdData((prev) => (prev[name] ? prev : { ...prev, [name]: [] }));
  const createDistrict = (divisionName: string, districtName: string) =>
    setBdData((prev) => {
      const list = prev[divisionName] ?? [];
      if (list.some((d) => d.জেলা.toLowerCase() === districtName.toLowerCase())) return prev;
      return { ...prev, [divisionName]: [...list, { জেলা: districtName, উপজেলা: [] }] };
    });
  ...
  const removeItem = (lineId: string) => dispatch(removeLocalItem({ id: lineId }));

  
  const validate = () => {
    const required: (keyof DeliveryForm)[] = ["customer_name", "customer_phone", "division", "district", "thana", "address"];
  ...
  const payment_type_api = form.payment_type === "cod" ? "CASH_ON_DELIVERY" : "FULL_PAYMENT";
  ...
  const placeOrder = async () => {
    ...
    const productsBody = items.map((it) => {
      ...
      const v = (p.variants || []).find((vv) => vv._id === lines.find((l) => l.id === it.id)?.variantId)!;
    ...
    const payload = {
      ...
      order_note: `${form.order_note || ""}\n[Payment] Sender: ${pay.senderPhone}, Amount: ${pay.amount}, Trx: ${pay.trxId}`,
    } as const;
        ...
        <div className="text-center max-w-md px-6">
          ...
          {orderPlaced.id && (
            <p className="text-gray-600 mb-6">
              Your order id: <span className="font-medium">{orderPlaced.id}</span>
            </p>
          )}
          <div className="flex items-center justify-center gap-3">
            <Link href="/"><Button className="bg-[#ff5c00] hover:bg-[#ff5c00]/90 text-white">Back to Home</Button></Link>
            <Link href="/cart"><Button variant="outline" className="border-[#ff5c00] text-[#ff5c00] hover:bg-[#ff5c00] hover:text-white">View Cart</Button></Link>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="bg-gradient-to-b from-white via-[#fff8f4] to-white">
      <Container className="py-6">
        {isError && (
          <div className="mb-4 flex items-center gap-2 rounded-md border border-red-200 bg-red-50 p-3 text-sm text-red-700">
            <Info className="h-4 w-4" />
            Could not validate products at the moment. You can still review your cart.
          </div>
        )}
              ...
              <CardContent className="space-y-4">
                {cartLoading ? (
                  ...
                  <>
                    ...
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                      <div>
                        <Label className="mb-2 block text-gray-600 text-xs">Full Name *</Label>
                        <Input
                      ...
                      <div>
                        <Label className="mb-2 block text-gray-600 text-xs">Phone Number *</Label>
                        <Input
                          type="tel"
                          className="h-12 rounded-lg focus-visible:ring-[#ff5c00]"
                    ...
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                      <div>
                        <Label className="mb-2 block text-gray-600 text-xs">Division *</Label>
                        <SuggestionDropdown
                          value={form.division}
                      ...
                      <div>
                        <Label className="mb-2 block text-gray-600 text-xs">District *</Label>
                        <SuggestionDropdown
                          value={form.district}
                          ...
                          creatable
                          onCreate={(name) => {
                            if (!form.division) return toast.error("Select a division first");
                      ...
                      <div>
                        <Label className="mb-2 block text-gray-600 text-xs">Thana / Upazila *</Label>
                        <SuggestionDropdown
                          value={form.thana}
                    ...
                    <div>
                      <Label className="mb-2 block text-gray-600 text-xs">Delivery Point *</Label>
                      <Input
                        className="h-12 rounded-lg focus-visible:ring-[#ff5c00]"
                    ...
                    <div>
                      <Label className="mb-2 block text-gray-600 text-xs">Order Note</Label>
                      <Textarea
                    ...
                    {/* Payment method */}
                    <div className="pt-2">
                      <Label className="mb-2 block text-gray-600 text-xs">Payment Type</Label>
                      <div className="grid grid-cols-1 gap-2 md:grid-cols-2">
                        <button
                          type="button"
                          onClick={() => setField("payment_type", "cod")}
                          className={`rounded-lg border px-4 py-3 text-sm ${form.payment_type === "cod"
                        ...
                        <button
                          ...
                          className={`rounded-lg border px-4 py-3 text-sm ${form.payment_type === "full"
                            ? "border-[#ff5c00] bg-[#ff5c00]/5 text-[#ff5c00]"
                            : "border-gray-300 hover:border-[#ff5c00]"
                            }`}
                        >
