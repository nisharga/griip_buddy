...
import { selectCartItems, removeItem as removeLocalItem, updateQty as updateLocalQty } from "@/redux/features/cart-slice";
...
const CartItemComponent: React.FC<{
  ...
  const onInputChange = (value: string) => setDraftQty(value.replace(/\D+/g, ""));
    ...
    <Card className="group relative rounded-2xl border border-gray-200 shadow-none bg-white transition-shadow hover:shadow-md">
      <CardContent className="px-4 py-3 md:px-5 md:py-0">
        <div className="grid grid-cols-[88px_1fr_auto] sm:grid-cols-[104px_1fr_auto] items-start gap-4">
          <div className="relative aspect-square w-[88px] sm:w-[104px] overflow-hidden rounded-xl bg-gray-50 ring-1 ring-gray-200">
            <Image
          ...
          <div className="min-w-0 space-y-2">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <h3 className="line-clamp-2 text-sm sm:text-base font-medium text-gray-900">{title}</h3>
                {item.variant?.sku && (
                  <p className="mt-0.5 text-xs text-gray-500">SKU: {item.variant.sku}</p>
                )}
              </div>
...
const CartSummary: React.FC<{ itemCount: number; totalPrice: number; isFetching?: boolean }> = ({
  itemCount,
  totalPrice,
  isFetching,
}) => {
    ...
    <Card className="sticky top-20 shadow-none border-gray-200">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2 pt-2">
          <Link href="/checkout">
            <Button className="w-full bg-[#ff5c00] hover:bg-[#ff5c00]/90 text-white text-sm py-3" size="lg">
...
const CartPage: React.FC = () => {
  ...
  const {
    ...
    isFetching,
    isError,
  } = useGetProductByIdsQuery({ product_ids: productIds }, { skip: productIds.length === 0 });
  ...
  const viewItems: CartItemView[] = useMemo(() => {
    ...
    for (const line of lines) {
      ...
      const unitPrice = Number(v.sale_price ?? v.regular_price ?? line.priceSnapshot ?? 0);
      out.push({
        id: line.id,
        product: { _id: p._id, name: p.name, thumbnail: p.thumbnail },
        variant: v,
        attributes: (v.attribute_values ?? line.attributes ?? {}) as Record<string, string>,
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between mb-6">
                    <h2 className="text-lg font-medium text-gray-900">
                      Cart Items <span className="text-[#ff5c00]">({viewItems.length})</span>
                    </h2>
                  ...
                  {viewItems.map((item) => (
                    <CartItemComponent
                      key={item.id}
                      item={item}
                      onUpdateQty={(id, q) => dispatch(updateLocalQty({ id, quantity: q }))}
                      onRemove={(id) => {
