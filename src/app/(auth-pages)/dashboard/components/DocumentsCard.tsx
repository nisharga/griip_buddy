...
const DocumentsCard: React.FC<DocumentsCardProps> = ({ documents }) => {
  const documentItems = [
    {
      ...
      status: documents?.NID?.front || documents?.NID?.back ? "partial" : "missing",
      details: `Front: ${documents?.NID?.front ? "✓" : "✗"}, Back: ${documents?.NID?.back ? "✓" : "✗"}`,
    },
    {
      name: "Passport",
      status: documents?.passport?.front || documents?.passport?.back ? "partial" : "missing",
      details: `Front: ${documents?.passport?.front ? "✓" : "✗"}, Back: ${documents?.passport?.back ? "✓" : "✗"}`,
    },
    {
  ...
  const completedCount = documentItems.filter((item) => item.status === "complete").length
