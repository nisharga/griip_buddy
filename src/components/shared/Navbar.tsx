...
// import { openCart, toggleCart } from "@/redux/features/cart-slice";
import CartSheet from "@/components/cart/cart-sheet";
import MobileSearch from "./MobileSearch";
import { popoverVariants, mobileMenuVariants } from "@/types/navbar";
import { useSearchProductsDropdownQuery } from "@/redux/api/publicApi";
import { IProductFull } from "@/types/IProductFull";
import {
  getUser,
  removeUser,
  removeVendorId,
} from "@/app/(auth-pages)/store/user";
import { removeAccessToken } from "@/lib/cookies";
import { useGetAllSubCategoriesQuery } from "@/redux/api/subcategory-api";
import { useGetCurrentProfileQuery } from "@/redux/api/auth-api";
        ...
        <Container>
          <div className='flex items-center justify-between py-1 gap-4'>
            <div className='flex w-full md:w-1/2 items-center justify-between'>
              <Link
                href='/'
                className='text-2xl md:text-3xl font-extrabold text-primary tracking-wide flex-shrink-0'>
                <Image
                  src={"/logo/logo.png"}
                  alt='Main logo of Griipbuddy'
                  className='md:h-[70px] h-[60px] w-auto'
                  width={400}
                  height={100}
                />
              </Link>
            ...
            <div className='hidden md:flex items-center gap-4 flex-shrink-0'>
                  ...
                  <AnimatePresence>
                    {isSearchPopoverOpen && isLoading ? (
                      ...
                      <motion.div
                        ...
                        <ul className='py-1'>
                          {searchData?.slice(0, 6).map((item: IProductFull) => (
                            <Link
                              key={item?._id}
              ...
              <div className='relative' ref={loginRef}>
                <button
                  ...
                  <span>Account</span>
                  <ChevronDown
                    className={`h-3 w-3 transition-transform ${isLoginDropdownOpen ? "rotate-180" : ""
                      }`}
                  />
                </button>
                <AnimatePresence>
                        ...
                        <ul className='py-1'>
                          ...
                          <li>
                            <button
                              ...
                              onClick={() => (setIsLoginDropdownOpen(false), handleLogOut())}>
                              Logout
                            </button>
                          </li>
                        </ul>
                ...
                {categoriesData.map((category) => (
                  <li
                    ...
                    <Link
                      ...
                      aria-expanded={hoveredCategory === category.name ? "true" : "false"}>
                      {category.name}
                      {category.subcategories.length > 0 && (
                        <ChevronDown
                          className={`ml-1 h-3 w-3 transition-transform ${hoveredCategory === category.name ? "rotate-180" : ""
                            }`}
                        />
                      )}
        ...
        <AnimatePresence>
          {isMobileMenuOpen && (
            <motion.div
              ...
              <ul className='flex flex-col gap-4 flex-grow mb-5'>
                {categoriesData.map((category) => (
                    ...
                    {category.subcategories.length > 0 ? (
                      <Collapsible
                        ...
                        <CollapsibleTrigger className='flex w-full items-center justify-between text-lg font-medium text-white hover:text-primary transition-colors py-2'>
                          {category.name}
                          <ChevronDown
                            className={`h-5 w-5 transition-transform ${openMobileSubcategory === category.name ? "rotate-180" : ""
                              }`}
                          />
                        </CollapsibleTrigger>
